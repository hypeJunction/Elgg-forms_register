...
SETTINGS_VALUES
    ...
    php -r "
        ...
        // Priority: elgg-plugin.php 'plugin.dependencies' (Elgg 7.x) then manifest.xml <requires type='plugin'>.
    ...
    # Enable public registration so the /register page works in tests.
    php -r "
        require_once 'vendor/autoload.php';
        \$app = \Elgg\Application::getInstance();
        \$app->bootCore();
        elgg_save_config('allow_registration', true);
        echo 'Registration enabled.' . PHP_EOL;
    " 2>&1
